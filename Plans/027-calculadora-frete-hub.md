...
**Origem:** a PDP mostra duas calculadoras (a nossa e a do plugin); o preço da nossa sai com entidade HTML (`&#82;&#36;`). A loja vai usar Virtuaria Correios e Melhor Envio; a UI de frete da loja precisa ser uma só e consultar todos os métodos WooCommerce ativos.  
...
|---|---|---|
| PDP — nossa | `ProductDetails::calculateShipping()` chama `WC()->shipping()->calculate_shipping()` | Já é um hub; o preço passa por `wc_price()` + `textContent` e aparece `&#82;&#36;&nbsp;41,26` |
...
- Resultado da PDP: lista com nome do método, preço em `R$` legível e prazo quando o método enviar prazo. Prazo de produção do produto continua separado (013).
...
| Preço | Enviar valor numérico + texto já formatado sem HTML, ou formatar no JS. Nunca `wc_price()` dentro de `textContent`. |
