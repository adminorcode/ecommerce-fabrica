...
3. Se a stack estiver down, a imagem estiver velha, Dockerfiles/lockfiles mudaram, ou o runtime não tiver o código: rode o comando canônico **em background** (`background: true`) para retornar ao agente sem matar o watch. `--watch` é processo long-running em primeiro plano; não use `-d` junto.
